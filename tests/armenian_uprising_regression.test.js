...
test("Armenian Uprising event offers blue-A placement choices and finishes after placing unit plus three markers", () => {
	let game = rules.setup(104, "Historical", { seed: 42 })
	ensureCommonState(game)

	let event = Engine.events.get_event_by_id(32)
	let unitSpace = findSpace("Van")
	let markerSpace1 = findSpace("Sivas")
	let markerSpace2 = findSpace("Erzurum")
	let armenian = findPieceIndex((piece) => piece.name === "Armenian Uprising")

	expect(unitSpace).toBeGreaterThan(0)
	expect(markerSpace1).toBeGreaterThan(0)
	expect(markerSpace2).toBeGreaterThan(0)
	expect(armenian).toBeGreaterThan(0)

	event.handler(game)

	let unitView = rules.view(game, AP_ROLE)
	let unitChoices = unitView.actions.space || []
	expect(unitChoices).toContain(unitSpace)
	expect(unitChoices).toContain(markerSpace1)
	expect(unitChoices).toContain(markerSpace2)

	game = rules.action(game, AP_ROLE, "space", unitSpace)
	expect(game.state).toBe("event_armenian_uprising_markers")
	expect(game.pieces[armenian]).toBe(unitSpace)
	expect(game.armenian_uprising_markers).toEqual([unitSpace])

	let markerView = rules.view(game, AP_ROLE)
	let markerChoices = markerView.actions.space || []
	expect(markerChoices).not.toContain(unitSpace)
	expect(markerChoices).toContain(markerSpace1)
	expect(markerChoices).toContain(markerSpace2)

	game = rules.action(game, AP_ROLE, "space", markerSpace1)
	game = rules.action(game, AP_ROLE, "space", markerSpace2)

	expect(game.state).toBe("confirm_event")
	expect(game.armenian_uprising_markers).toEqual([unitSpace, markerSpace1, markerSpace2])
})
