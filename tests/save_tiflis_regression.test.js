 ...
 *   - 无法合法撤退的单位被消灭，并设置 save_tiflis_failed = true
...
const AP_ELIMINATED = data.spaces.findIndex((s, i) => i > 0 && s && s.name === "AP Eliminated")
...
test("SAVE TIFLIS: cannot_retreat 将单位消灭并设置 save_tiflis_failed", () => {
	const game = makeSaveTiflisRetreatGame([RU_CAV_1])
	game.pieces[RU_CAV_1] = TIFLIS
	...
	expect(game.pieces[RU_CAV_1]).toBe(AP_ELIMINATED)
