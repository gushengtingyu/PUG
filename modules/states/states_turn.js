...
exports.register = function (states, Engine, context) {
		...
		if (game.oos && game.oos.length > 0) {
			...
			for (let p of oos_units) {
				if (set_has(game.reduced, p)) {
					...
					eliminate_piece(p, true)
				} else if (data.pieces[p].rcf !== undefined) {
					log(`${data.pieces[p].name} reduced (OOS)`)
					set_add(game.reduced, p)
				} else {
					log(`${data.pieces[p].name} eliminated (OOS)`)
					eliminate_piece(p, true)
	...
	function start_mandated_offensive_phase() {
		...
		if (game.mo_cp_cancelled) {
			log("CP MO: Cancelled for rest of game")
			game.mo_cp = MO_NONE
			game.mo_cp_fulfilled = true
		} else {
			log(`MO Roll: AP ${ap_roll}, CP ${cp_roll}`)
			...
			while (status_ap === "REROLL") {
				...
				log(`AP MO Area empty, rerolling: ${ap_roll} -> ${mo_ap}`)
			}
			if (status_ap === "NONE") {
				log(`AP MO ${mo_ap} nation not on map: None`)
				mo_ap = MO_NONE
			}
		...
		if (game.mo_ap === MO_RUSSIA && game.events["russian_revolution"]) {
			log("MO AP: Russia ignored after Russian Revolution begins")
			game.mo_ap = MO_NONE
		}
		if (game.mo_cp === MO_RUSSIA && game.events["russian_revolution"] >= 4) {
			log("MO CP: Russia ignored after Russian Revolution Stage 4")
			game.mo_cp = MO_NONE
		}

		if (game.mo_ap !== MO_NONE) {
			if (game.mo_ap === MO_AP_CHOICE_5) {
				log("MO AP: Choice needed")
			} else {
				log(`MO AP: ${game.mo_ap}`)
			}
	...
	function start_war_status_phase() {
		...
		if (game.blockade_vp_penalty_active && get_season(game) === "Winter") {
			...
			if (total_war_cp && railway_complete) {
				...
				log(
					"Berlin-Constantinople railway complete and CP in Total War: Royal Navy Blockade VP penalty removed."
				)
			} else {
				game.vp -= 1
				log("Royal Navy Blockade: -1 VP (Winter Phase).")
			}
		}
	...
	function check_war_commitment_increase() {
		...
		if (game.war_commitment_ap === COMMITMENT_MOBILIZATION && game.war_status_ap >= 4) {
			...
			log("AP War Commitment increases to LIMITED WAR")
			...
			log("AP War Commitment increases to TOTAL WAR")
		...
		if (game.war_commitment_cp === COMMITMENT_MOBILIZATION && game.war_status_cp >= 4) {
			...
			log("CP War Commitment increases to LIMITED WAR")
			...
			log("CP War Commitment increases to TOTAL WAR")
	...
	states.rp_phase = {
		...
		piece(p) {
			...
			if (is_eliminated(game, p)) {
				log(`${faction_name(game.active)} 重组 ${info.name}`)
