...
module.exports = function (Engine) {
	...
	function apply_severe_weather(game, log, season, reduce_piece_fn) {
		...
		if (pugnacity_active) {
			let has_indian_attacker = game.attack && game.attack.pieces.some((p) => get_piece_nation(p) === "in")
		...
		let is_ru_attacking = game.attack.pieces.some((p) => get_piece_nation(p) === "ru")
	...
	function get_advance_pieces(game, result) {
		...
		let eligible = attackers.filter((p) => {
			...
			if (save_tiflis_override) {
				...
				let is_tu_tua = nation === "tu" || nation === "tua"
	...
	function can_declare_turkish_retreat(game) {
		...
		let adj = get_connected_spaces(game, space, get_piece_nation(p), CP, p)
	...
	function get_attackable_spaces(game, pieces, faction, get_season_fn, is_rail_connected_to_supply_fn) {
		...
		for (let p of pieces) {
			...
			let adj = get_connected_spaces(game, s, get_piece_nation(p), get_piece_faction(p), p)
			let targets = []
			for (let t of adj) {
	...
	function can_activate_piece_in_space_to_attack(game, p, s, faction, get_season_fn, is_rail_connected_to_supply_fn) {
		...
		let is_lcu_p = is_lcu(p)
		let is_balkan_only = data.pieces[p].region_limit === "B"
		for (let t of adj) {
	...
	function resolve_battle_sequence(game, context) {
		...
		// Restore active player to attacker (who started the sequence)
			...
			if (game.combat_cards) {
				...
				const return_to_hand = (card) => {
					...
					if (effected_cards.includes(card)) {
						if (log_fn) log_fn(`战斗取消: ${data.cards[card].name} 已产生效果，不返回.`)
						return
					}
					...
					if (was_retained) {
						...
						if (log_fn) log_fn(`战斗取消: 将 ${info.name} 返回至 ${owner.toUpperCase()} 战场保留区.`)
						...
						if (set_has(discard, card)) {
							...
							set_add(hand, card)
							if (log_fn) log_fn(`战斗取消: 将 ${info.name} 返回至 ${owner.toUpperCase()} 手牌.`)
						} else if (set_has(removed, card)) {
							set_delete(removed, card)
							set_add(hand, card)
							if (log_fn) log_fn(`战斗取消: 将 ${info.name} (星号卡) 返回至 ${owner.toUpperCase()} 手牌.`)
						}
					}
				}

				if (game.combat_cards.attacker) {
					game.combat_cards.attacker.forEach((c) => return_to_hand(c, attacker_faction))
				}
				if (game.combat_cards.defender) {
					game.combat_cards.defender.forEach((c) => return_to_hand(c, defender_faction))
				}
			}
	...
	function retain_winning_combat_cards(game, result, log_fn) {
		...
		for (let c of played) {
			...
			if (log_fn) log_fn(`Retained CC: ${info.name}`)
		}
	}
	...
	function end_battle_sequence(game, log_fn) {
		...
		if (!result || result.cancelled) {
			// Clear temp context if not keeping it for an event
			if (!game.attack.keep_context) {
		...
		let defender_faction = game.attack.defender || other_faction(attacker_faction)
			...
			if (winner) {
				...
				for (let p of result.used_hqs.defender) {
					if (winner === other_faction(game.active)) {
						...
						// Rule 16.3.5: Army of Islam bonus (defender wins)
						if (data.pieces[p].name.includes("Army of Islam")) {
							let space = data.spaces[target_space]
							if (space.tribal) {
								log_fn(`Rule 16.3.5: Army of Islam won in a tribal space, +1 TU RP bonus.`)
		...
		// Rule 16.4: Heavy Artillery flips/removal
		...
		// Rule 12.6.8: Elimination of HQs if all friendly units are gone
		const check_12_6_8 = (space, faction) => {
			...
			if (pieces.length > 0) {
				...
				if (combat_units.length === 0) {
					...
					for (let p of hqs) {
						log_fn(`Rule 12.6.8: ${data.pieces[p].name} is eliminated because no combat units remain in its space.`)
						eliminate_piece(game, p, log_fn, true)
					}
				}
			}
		...
		if (check_save_tiflis_event(game, log_fn)) {
			// Save Tiflis event handles its own state transitions and cleanup
			delete game.battle_result
			return
		}
		...
		if (result.retreat_needed && defenders_in_space.length > 0) {
			let retreating = result.retreating_units.filter(
		...
		if (!result.retreat_needed || defenders_in_space.length === 0) {
			...
			if (defenders_in_space.length === 0) {
				...
				if (!result.no_advance || game.save_tiflis_failed) {
					if (should_offer_post_battle_cc_window(game)) {
						game.active = CP
						game.post_battle_cc_resume = {
							kind: "advance",
					...
					game.advance_pieces = get_advance_pieces(game, result)
	...
	function get_valid_retreat_spaces(
		...
		let find_valid_steps = (from_space, steps_left, local_avoided) => {
			let adj = get_connected_spaces(game, from_space, nation, faction, piece)
			let valid = []
			for (let next of adj) {
	...
	function resolve_russian_winter_offensive_advance(game, p, advance_space, log_fn) {
		if (is_turn_event(game, "russian_winter_offensive") && game.active === AP && get_piece_nation(p) === "ru") {
	...
	function get_turkish_retreat_space(game, defenders) {
		...
		for (let next of neighbors) {
			...
			if (contains_enemy_pieces(game, next, faction)) continue
			if (has_undestroyed_fort(game, next, other_faction(faction)) || has_undestroyed_fort(game, next, faction)) continue
	...
	function get_combat_card_drm(card, side, game, attackers, defenders) {
		...
		const has_nation = (pieces, nations) => pieces.some((p) => nations.includes(get_piece_nation(p)))
	...
	function check_can_flank(game) {
		...
		let empty_fort = has_undestroyed_fort(game, target_space, defender_faction) && !has_defenders && !is_besieged_val
	...
	function resolve_battle(game, log_fn) {
		...
		if (attempt_flank) {
			...
			log_detail(log, `牵制空间：${space_name(pinning_space)}`)
			...
			if (flank_final >= 4) {
				log(`侧翼进攻成功！（掷骰：${flank_roll} + ${flank_drm} = ${flank_final}）`)
				att_fire_first = true
			} else {
				log(`侧翼进攻失败。（掷骰：${flank_roll} + ${flank_drm} = ${flank_final}）`)
				def_fire_first = true
			}
		} else if (can_flank) {
			...
			if (attacking_spaces_count >= 2) {
				...
				if (!has_lcu_val) {
					log_detail(log, "无法进行侧翼进攻：没有 LCU 参与进攻。")
				} else if (target_terrain === MOUNTAIN || target_terrain === SWAMP) {
					log_detail(log, `无法在 ${target_terrain} 地形进行侧翼进攻。`)
				} else if (trench_level > 0 && !ignore_trench) {
					log_detail(log, "无法对有战壕的空间进行侧翼进攻。")
				} else if (empty_fort_val) {
					log_detail(log, "无法对空的非围攻要塞进行侧翼进攻。")
				} else if (is_river_def_val && !ignore_trench) { // Jihad ignore covers river too
					log_detail(log, "无法进行侧翼进攻：所有攻击均跨越河流/海峡。")
				}
			}
		}
		...
		if (game.active === AP && is_naval_access_space(game, target_space)) {
			...
			} else if (attackers.some((p) => ["br", "anz", "in"].includes(get_piece_nation(p)) && is_lcu(p))) {
				att_drm += 1
		...
		// 6. Event DRMs
		if (game.active === AP) {
			let is_ru_attacking = attackers.some((p) => get_piece_nation(p) === "ru")
			if (is_ru_attacking) {
			...
			let is_tu_attacking = attackers.some((p) => ["tu", "tua"].includes(get_piece_nation(p)))
			...
			if (game.indian_mutiny_drm && game.active === CP) {
				let has_indian_defenders = defenders.some((p) => get_piece_nation(p) === "in")
		...
		if (game.combat_cards) {
			...
			if (game.combat_cards.defender && game.combat_cards.defender.includes(CC_CP_CATASTROPHIC_ATTACK)) {
				att_shifts -= 1
				att_shift_factors.push("-1 灾难性攻击")
				mark_effected(CC_CP_CATASTROPHIC_ATTACK)
				log_detail(log, "Catastrophic Attack: Shift 1 Left")
			}
		}
		...
		if (def_losses > att_losses) {
			...
			if (retreating_faction === AP) {
				...
				let has_yudenitch = pieces_in_space.some((p) => data.pieces[p].name === "RU Yudenitch HQ" && !is_eliminated(game, p))
				let has_ru_units = pieces_in_space.some((p) => data.pieces[p].nation === "ru" && !is_hq(p) && !is_eliminated(game, p))
	...
	function resolve_second_fire(game, log_fn) {
		...
		if (result.att_fire_first) {
			...
			let def_table_type = def_table === fire_table.lcu ? "lcu" : "scu"
			let def_roll = roll_die(6, game)
			let def_final_roll = Math.max(1, Math.min(6, def_roll + result.def_drm))
			...
			let att_table_type = att_table === fire_table.lcu ? "lcu" : "scu"
			let att_roll = roll_die(6, game)
			let att_final_roll = Math.max(1, Math.min(6, att_roll + result.att_drm))
